...
docker compose up -d database && sleep 5
docker compose exec -T database psql -U partdb -d postgres \
  -c "DROP DATABASE IF EXISTS partdb;" -c "CREATE DATABASE partdb OWNER partdb;"
docker compose exec -T database pg_restore -U partdb -d partdb < "$SRC/partdb.dump"
